...
const Chat = (props) => {
    ...
    }, [route.params?.notification, route.params?.user]);
        ...
        navigation.setOptions({
            headerTitle: () => (
                <View style={[container.horizontal, utils.alignItemsCenter, { overflow: 'hidden' }]}>
                    ...
                    <Text style={[text.bold, text.large, { flex: 1 }]} numberOfLines={1} ellipsizeMode='tail'>{route.params?.user?.username}</Text>
                </View>
            ),
        });
        if (chat !== undefined) {
                            ...
                            {item?.creator !== undefined && item?.creation !== null ?
                                <View style={container.horizontal}>
                                    <View>
                                        ...
                                        {item?.post != null ?
                                            <TouchableOpacity style={{ marginBottom: 20, marginTop: 10 }} onPress={() => { navigation.navigate("Post", { item: item?.post, user: item?.post.user }) }}>
                                                {item?.post?.type == 0 ?
                                                    <CachedImage
                                                        cacheKey={item?.id}
                                        ...
                                        <Text
                                            ...
                                            {formatDate(item?.creation?.seconds)}
                                        </Text>
                                    </View>
                                </View>
                                : null}
